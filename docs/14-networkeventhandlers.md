...
For example, consider the default `announce` command, which creates a message on every player's screen. By default, this creates a system chat message with the given text, because CmdrX has a default event handler for the `"Message"` event, which the `announce` command broadcasts.

If you wanted to display announcements some other way, you could just override the default event handler:

```lua
CmdrXClient:HandleEvent("Message", function (text, player)
